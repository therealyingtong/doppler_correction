...
class KeyGenerator:
    ...
    def calcG2Shift(self, tau = 10000):
        ...
        def pad(arr,t):
            ...
            return list(numpy.array(paddedArray))
        
        def fillArray(arr1, arr2):
        ...
        bob = pad(self.timeStampBob, self.tau)
        alice = pad(self.timeStampAlice, self.tau)

        
       # [alice, bob] = pad([self.timeStampAlice, self.timeStampBob], self.tau)
        [alice, bob] = fillArray(alice, bob)
    ...
    def determineStart(self):
        ...
        t2 = self.convertStamp(fAlice.readline())
...
if __name__ == "__main__":
    k = KeyGenerator("ALICE_12Apr_19_3", "BOB_12Apr19_3", 'bin64')
    k.setStartTime()
    k.determineStart()
    k.stampPreparation()
    k.calcG2Shift(10000)
